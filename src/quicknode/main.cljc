(ns quicknode.main
  "Kotodama WASM entrypoint for the Quicknode clean-room actor (L4) — Clojure port.
  ...
  facts (`quicknode.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "quicknode")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"quicknode.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "quicknode-compat" :tier tier :entities entities} 200])
...
(defn start! [] :quicknode-compat/ready)
